...
class SalesBookFiscalMachineReportCustomHandler(models.AbstractModel):
    ...
    def _get_retention_iva_values(self, move, options):
        ...
        if not hasattr(move, 'retention_iva_line_ids') or not move.retention_iva_line_ids:
            return {
                'date_retention': '',
                'number_retention': '',
                'iva_retained': 0.0,
        ...
        ret_lines = move.retention_iva_line_ids.filtered(
            lambda x: x.retention_id.state == 'emitted'
        ) if move.state == 'posted' else move.retention_iva_line_ids
        ...
        for ret_line in ret_lines:
            ...
            if date_from and retention.date_accounting:
                if retention.date_accounting < date_from or retention.date_accounting > date_to:
                    continue
            
            if ret_vals['date_retention']:
                # If we already have a date, append the new one (comma-separated)
                ret_vals['date_retention'] += ', ' + format_date(self.env, retention.date, date_format='dd/MM/yyyy')
            else:
                ret_vals['date_retention'] = format_date(self.env, retention.date, date_format='dd/MM/yyyy') if retention.date else ''
            
            if not ret_vals['number_retention']:
                ret_vals['number_retention'] = move.iva_voucher_number if hasattr(move, 'iva_voucher_number') else ''
    ...
    def _custom_options_initializer(self, report, options, previous_options):
        ...
        for column in options.get('columns', []):
            ...
            if col_expr_label in ['tax_base_general_aliquot', 'general_aliquot', 'amount_general_aliquot']:
                should_include = hasattr(company, 'general_aliquot_sale') and company.general_aliquot_sale
            elif col_expr_label in ['tax_base_reduced_aliquot', 'reduced_aliquot', 'amount_reduced_aliquot']:
                should_include = hasattr(company, 'reduced_aliquot_sale') and company.reduced_aliquot_sale
            elif col_expr_label in ['tax_base_extend_aliquot', 'extend_aliquot', 'amount_extend_aliquot']:
                should_include = hasattr(company, 'extend_aliquot_sale') and company.extend_aliquot_sale
    ...
    def action_open_group_moves(self, options, params):
        ...
        _logger.info("Report ID: %s, exists: %s", options.get('report_id'), report.exists())
        ...
        _logger.info("Group characteristics - Serial: %s, Report Z: %s", serial_number, report_z)
        _logger.info("First move create_date: %s, invoice_date: %s", first_move.create_date, first_move.invoice_date)
        ...
        if first_move.create_date:
            ...
            date_start = first_move.create_date.replace(hour=0, minute=0, second=0, microsecond=0)
            date_end = first_move.create_date.replace(hour=23, minute=59, second=59, microsecond=999999)
            date_domain = [
        ...
        if multi_company:
            domain.append(('company_id', 'in', [c['id'] for c in multi_company if c.get('selected')]))
        else:
    ...
    def _get_tax_values_from_stored(self, move):
        ...
        percent_general_default = float(company.general_aliquot_sale.amount) if hasattr(company, 'general_aliquot_sale') and company.general_aliquot_sale else 16.0
        percent_reduced_default = float(company.reduced_aliquot_sale.amount) if hasattr(company, 'reduced_aliquot_sale') and company.reduced_aliquot_sale else 8.0
        percent_extend_default = float(company.extend_aliquot_sale.amount) if hasattr(company, 'extend_aliquot_sale') and company.extend_aliquot_sale else 31.0
    ...
    def _calculate_tax_values(self, move):
        ...
        if move.state != 'posted':
            company = move.company_id
            if company:
                result['percent_general'] = float(company.general_aliquot_sale.amount) if hasattr(company, 'general_aliquot_sale') and company.general_aliquot_sale else 16.0
                result['percent_reduced'] = float(company.reduced_aliquot_sale.amount) if hasattr(company, 'reduced_aliquot_sale') and company.reduced_aliquot_sale else 8.0
                result['percent_extend'] = float(company.extend_aliquot_sale.amount) if hasattr(company, 'extend_aliquot_sale') and company.extend_aliquot_sale else 31.0
            else:
            ...
            for tax_info in tax_groups:
                ...
                for ttype, tg_id in tax_config.items():
                    if tg_id == tax_group_id:
                        base = tax_info.get('base_amount', tax_info.get('base_amount_currency', 0.0)) * multiplier
                        amount = tax_info.get('tax_amount', tax_info.get('tax_amount_currency', 0.0)) * multiplier
                        if ttype == 'exempt':
    ...
    def _dynamic_lines_generator(self, report, options, all_column_groups_expression_totals, warnings=None):
        ...
        for move_data in moves_data:
            move = move_data['move']
            key = str(move.create_date.strftime("%d-%m-%Y")) if move.create_date else str(move.invoice_date.strftime("%d-%m-%Y")) if move.invoice_date else ''
        ...
        for date_key, date_moves in agrouped_by_date.items():
            ...
            for move in sorted(date_moves, key=lambda m: int(m.l10n_ve_invoice_number) if m.l10n_ve_invoice_number and m.l10n_ve_invoice_number.isdigit() else 0):
                key = str(move.l10n_ve_serial_number or '') + "_" + str(move.l10n_ve_report_z or '')
            ...
            for report_key, report_moves in agrouped_by_report_z.items():
                ...
                for index, move in enumerate(report_moves):
                    ...
                    tax_values = self._get_tax_values_from_stored(move) if hasattr(move, 'sale_tax_data') and move.sale_tax_data else self._calculate_tax_values(move)
                    amounts = {
                    ...
                    cumulative = {
                        'tax_base_exempt_aliquot': cumulative['tax_base_exempt_aliquot'] + amounts.get('tax_base_exempt_aliquot', 0.0),
                        'amount_taxed': cumulative['amount_taxed'] + amounts.get('amount_taxed', 0.0),
                        'tax_base_reduced_aliquot': cumulative['tax_base_reduced_aliquot'] + amounts.get('tax_base_reduced_aliquot', 0.0),
                        'amount_reduced_aliquot': cumulative['amount_reduced_aliquot'] + amounts.get('amount_reduced_aliquot', 0.0),
                        'tax_base_general_aliquot': cumulative['tax_base_general_aliquot'] + amounts.get('tax_base_general_aliquot', 0.0),
                        'amount_general_aliquot': cumulative['amount_general_aliquot'] + amounts.get('amount_general_aliquot', 0.0),
                    }
                    ...
                    if move.move_type in ["out_invoice", "out_refund"]:
                        has_retention = hasattr(move, 'retention_iva_line_ids') and move.retention_iva_line_ids
                        if (hasattr(move.partner_id, 'taxpayer_type') and move.partner_id.taxpayer_type == "ordinary" and move.move_type == "out_invoice" and not has_retention):
                            if move not in group_moves_list:
                                group_moves_list.append(move)
                        if move.move_type == "out_invoice" and hasattr(move.journal_id, 'is_debit') and move.journal_id.is_debit:
                            line_dict = self._build_line_dict(report, move, tax_values, options, index + 1)
                            ...
                            continue
                        if (
                            (hasattr(move.partner_id, 'prefix_vat') and move.partner_id.prefix_vat == "J")
                            or (hasattr(move.partner_id, 'taxpayer_type') and move.partner_id.taxpayer_type != "ordinary")
                            ...
                            if cumulative['amount_taxed'] != amounts['amount_taxed']:
                                data = {
                                    ...
                                    "range_end": range_last if range_last != 0 else move.l10n_ve_invoice_number,
                                    "date": move.invoice_date,
                                    "report_z": move.l10n_ve_report_z,
                                    "serial_number": move.l10n_ve_serial_number,
                                }
                                group_line = self._build_group_line_dict(report, data, cumulative, options, group_moves_list.copy())
                        ...
                        if (
                            (
                                ...
                                or (hasattr(next_move.partner_id, 'prefix_vat') and next_move.partner_id.prefix_vat == "J")
                                or (hasattr(next_move.partner_id, 'taxpayer_type') and next_move.partner_id.taxpayer_type != "ordinary")
                                or next_move.move_type != "out_invoice"
                            )
                            or is_last_move
                        ) and (hasattr(move.partner_id, 'taxpayer_type') and move.partner_id.taxpayer_type == "ordinary") and not has_retention:
                            data = {
                                "move_type": move.move_type,
                                "range_start": range_start,
                        ...
                        if (
                            not is_last_move
                            and hasattr(move.partner_id, 'taxpayer_type') and move.partner_id.taxpayer_type == "ordinary"
                        ):
        ...
        separator_line = {
            ...
            'name': '',
            'columns': [report._build_column_dict(None, col, options=options) for col in options['columns']],
            'level': 0,
            'unfoldable': False,
    ...
    def _build_line_dict(self, report, move, tax_values, options, index):
        ...
        for column in options['columns']:
            ...
            if col_expr_label == 'index':
                ...
                date_str = format_date(self.env, move.invoice_date, date_format='dd/MM/yyyy') if move.invoice_date else ''
                line_columns.append(report._build_column_dict(
                    date_str,
                    column,
                    options=options,
                ...
                number_affected = move.reversed_entry_id.l10n_ve_invoice_number if (hasattr(move, 'reversed_entry_id') and move.reversed_entry_id and hasattr(move.reversed_entry_id, 'l10n_ve_invoice_number')) else ''
                line_columns.append(report._build_column_dict(
                    number_affected or '',
                    column,
                    options=options,
                ...
                line_columns.append(report._build_column_dict(None, column, options=options))
        
        line_dict = {
            'id': report._get_generic_line_id('account.move', move.id, markup='sales_book_fiscal_machine_line'),
            'name': move.name or '',
    ...
    def _build_group_line_dict(self, report, data, cumulative, options, group_moves=None):
        line_columns = []
        for column in options['columns']:
            col_expr_label = column['expression_label']
            if col_expr_label == 'index':
                ...
                company = self.env.company
                percent_reduced = float(company.reduced_aliquot_sale.amount) if hasattr(company, 'reduced_aliquot_sale') and company.reduced_aliquot_sale else 8.0
                line_columns.append(report._build_column_dict(
                    percent_reduced,
                    column,
                    options=options,
                ...
                company = self.env.company
                percent_general = float(company.general_aliquot_sale.amount) if hasattr(company, 'general_aliquot_sale') and company.general_aliquot_sale else 16.0
                line_columns.append(report._build_column_dict(
                    percent_general,
                    column,
                    options=options,
                ...
                company = self.env.company
                percent_extend = float(company.extend_aliquot_sale.amount) if hasattr(company, 'extend_aliquot_sale') and company.extend_aliquot_sale else 31.0
                line_columns.append(report._build_column_dict(
                    percent_extend,
                    column,
                    options=options,
        ...
        line_dict = {
            'id': report._get_generic_line_id('account.move', first_move_id, markup='sales_book_fiscal_machine_group_line'),
    ...
    def _generate_resume_lines(self, report, options, moves_data):
        ...
        resume_sections = [
            ...
            {'name': _('Adjustments to Tax Debits from Previous Periods'), 'key': 'adjustments'},
            {'name': _('Total Sales and Tax Debits for the Period'), 'key': 'total', 'is_total': True},
        ]
        
        for section in resume_sections:
            section_data = resume_data.get(section['key'], {
                'base_invoices': 0.0,
                'amount_invoices': 0.0,
                'base_credits': 0.0,
                'amount_credits': 0.0,
            ...
            line_dict = {
                'id': report._get_generic_line_id(None, None, markup=f"resume_{section['key']}"),
                'name': section['name'],
                'columns': line_columns,
    ...
    def _calculate_resume_data(self, moves, company):
        result = {
            'exempt': {'base_invoices': 0.0, 'amount_invoices': 0.0, 'base_credits': 0.0, 'amount_credits': 0.0},
            'exports_general': {'base_invoices': 0.0, 'amount_invoices': 0.0, 'base_credits': 0.0, 'amount_credits': 0.0},
            'exports_extend': {'base_invoices': 0.0, 'amount_invoices': 0.0, 'base_credits': 0.0, 'amount_credits': 0.0},
            'general': {'base_invoices': 0.0, 'amount_invoices': 0.0, 'base_credits': 0.0, 'amount_credits': 0.0},
            'reduced': {'base_invoices': 0.0, 'amount_invoices': 0.0, 'base_credits': 0.0, 'amount_credits': 0.0},
            'adjustments': {'base_invoices': 0.0, 'amount_invoices': 0.0, 'base_credits': 0.0, 'amount_credits': 0.0},
            'total': {'base_invoices': 0.0, 'amount_invoices': 0.0, 'base_credits': 0.0, 'amount_credits': 0.0},
        }
        
        invoices = [m for m in moves if m.move_type == 'out_invoice' and m.state == 'posted']
        credit_notes = [m for m in moves if m.move_type == 'out_refund' and m.state == 'posted']
        ...
        for move in invoices:
            ...
            result['general']['amount_invoices'] += tax_values.get('amount_general', 0.0)
            result['reduced']['base_invoices'] += tax_values.get('base_reduced', 0.0)
            result['reduced']['amount_invoices'] += tax_values.get('amount_reduced', 0.0)
        
        for move in credit_notes:
            tax_values = self._get_tax_values_from_stored(move) if hasattr(move, 'sale_tax_data') and move.sale_tax_data else self._calculate_tax_values(move)
            
            result['exempt']['base_credits'] += abs(tax_values.get('total_exempt', 0.0))
            result['general']['base_credits'] += abs(tax_values.get('base_general', 0.0))
            result['general']['amount_credits'] += abs(tax_values.get('amount_general', 0.0))
            result['reduced']['base_credits'] += abs(tax_values.get('base_reduced', 0.0))
            result['reduced']['amount_credits'] += abs(tax_values.get('amount_reduced', 0.0))
