...
class AccountTaxReportActivity(models.Model):
    ...
    def action_open_tax_activity(self):
        self.ensure_one()
        if self.activity_type_id == self.env.ref('l10n_ve_reports.mail_activity_type_tax_report_to_pay'):
            move = self.env['account.move'].browse(self.res_id)
            return move._action_tax_to_pay_wizard()
        elif self.activity_type_id == self.env.ref('l10n_ve_reports.mail_activity_type_tax_report_to_be_sent'):
            move = self.env['account.move'].browse(self.res_id)
        ...
        action = self.env["ir.actions.actions"]._for_xml_id("l10n_ve_reports.action_account_report_gt")
