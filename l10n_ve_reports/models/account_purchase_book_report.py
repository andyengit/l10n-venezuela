...
class PurchaseBookReportCustomHandler(models.AbstractModel):
    ...
    def _custom_options_initializer(self, report, options, previous_options):
        ...
        for column in options.get('columns', []):
            ...
            if col_expr_label in ['tax_base_general_aliquot', 'general_aliquot', 'amount_general_aliquot']:
                should_include = hasattr(company, 'general_aliquot_purchase') and company.general_aliquot_purchase
            elif col_expr_label in ['tax_base_reduced_aliquot', 'reduced_aliquot', 'amount_reduced_aliquot']:
                should_include = hasattr(company, 'reduced_aliquot_purchase') and company.reduced_aliquot_purchase
            elif col_expr_label in ['tax_base_extend_aliquot', 'extend_aliquot', 'amount_extend_aliquot']:
                should_include = hasattr(company, 'extend_aliquot_purchase') and company.extend_aliquot_purchase
    ...
    def _dynamic_lines_generator(self, report, options, all_column_groups_expression_totals, warnings=None):
        ...
        for move_data in moves_data:
            ...
            tax_values = self._get_tax_values_from_stored(move) if hasattr(move, 'purchase_tax_data') and move.purchase_tax_data else self._calculate_tax_values(move)
            ...
            for column in options['columns']:
                ...
                if col_expr_label == 'index':
                    ...
                    correlative = move.l10n_ve_control_number if hasattr(move, 'l10n_ve_control_number') else (move.correlative if hasattr(move, 'correlative') else '')
                    line_columns.append(report._build_column_dict(
                        correlative or '',
                        column,
                        options=options,
            ...
            line_dict = {
                'id': report._get_generic_line_id('account.move', move.id, markup='purchases_book_line'),
                'name': move.name or '',
    ...
    def _get_tax_values_from_stored(self, move):
        ...
        tax_config = {}
        if hasattr(company, 'exent_aliquot_purchase') and company.exent_aliquot_purchase:
            tax_config['exempt'] = company.exent_aliquot_purchase.tax_group_id.id
        if hasattr(company, 'reduced_aliquot_purchase') and company.reduced_aliquot_purchase:
            tax_config['reduced'] = company.reduced_aliquot_purchase.tax_group_id.id
        if hasattr(company, 'general_aliquot_purchase') and company.general_aliquot_purchase:
            tax_config['general'] = company.general_aliquot_purchase.tax_group_id.id
        if hasattr(company, 'extend_aliquot_purchase') and company.extend_aliquot_purchase:
            tax_config['extend'] = company.extend_aliquot_purchase.tax_group_id.id
        
        purchase_tax_data = move.purchase_tax_data
        
        percent_general_default = float(company.general_aliquot_purchase.amount) if hasattr(company, 'general_aliquot_purchase') and company.general_aliquot_purchase else 16.0
        percent_reduced_default = float(company.reduced_aliquot_purchase.amount) if hasattr(company, 'reduced_aliquot_purchase') and company.reduced_aliquot_purchase else 8.0
        percent_extend_default = float(company.extend_aliquot_purchase.amount) if hasattr(company, 'extend_aliquot_purchase') and company.extend_aliquot_purchase else 31.0
    ...
    def _calculate_tax_values(self, move):
        ...
        if move.state != 'posted':
            company = move.company_id
            if company:
                result['percent_general'] = float(company.general_aliquot_purchase.amount) if hasattr(company, 'general_aliquot_purchase') and company.general_aliquot_purchase else 16.0
                result['percent_reduced'] = float(company.reduced_aliquot_purchase.amount) if hasattr(company, 'reduced_aliquot_purchase') and company.reduced_aliquot_purchase else 8.0
                result['percent_extend'] = float(company.extend_aliquot_purchase.amount) if hasattr(company, 'extend_aliquot_purchase') and company.extend_aliquot_purchase else 31.0
            else:
        ...
        if tax_totals:
            total_taxed_amount = tax_totals.get('total_amount', tax_totals.get('total_amount_currency', 0.0))
    ...
    def _generate_resume_lines(self, report, options, moves_data):
        ...
        resume_sections = [
            ...
            {'name': _('Adjustments to Tax Credits from Previous Periods'), 'key': 'adjustments'},
            {'name': _('Total Purchases and Tax Credits for the Period'), 'key': 'total', 'is_total': True},
        ]
    ...
    def _calculate_resume_data(self, moves, company):
        result = {
            ...
            'imports_general': {'base_invoices': 0.0, 'amount_invoices': 0.0, 'base_credits': 0.0, 'amount_credits': 0.0},
            'imports_extend': {'base_invoices': 0.0, 'amount_invoices': 0.0, 'base_credits': 0.0, 'amount_credits': 0.0},
        ...
        invoices = [m for m in moves if m.move_type == 'in_invoice' and m.state == 'posted']
        credit_notes = [m for m in moves if m.move_type == 'in_refund' and m.state == 'posted']
