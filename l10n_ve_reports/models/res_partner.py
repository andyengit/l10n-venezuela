...
class ResPartner(models.Model):
    ...
    def open_partner_ledger(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id("l10n_ve_reports.action_account_report_partner_ledger")
        action['params'] = {
    ...
    def open_customer_statement(self):
        if not self.env.ref('l10n_ve_reports.customer_statement_report', raise_if_not_found=False):
            return self.open_partner_ledger()
        action = self.env["ir.actions.actions"]._for_xml_id("l10n_ve_reports.action_account_report_customer_statement")
        action['params'] = {
            'options': {
