...
class AccountMove(models.Model):
    ...
    def _tfhka_get_serie(self):
        self.ensure_one()
        serie = re.sub(r"[^0-9A-Za-z]", "", (self.journal_id.series_correlative or "").strip())
        return serie[:20]
    ...
    def _tfhka_validate_identificacion_documento(self):
        ...
        if not digits_only or not digits_only.isdigit() or len(digits_only) > 19:
            raise UserError(
                "NumeroDocumento debe tener entre 1 y 19 digitos (puede incluir un guion, p. ej. 00000023-0)."
            )
    ...
    def _tfhka_get_affected_invoice_serie(self):
        ...
        if not affected_move:
            return ""
        serie = re.sub(
            r"[^0-9A-Za-z]", "", (affected_move.journal_id.series_correlative or "").strip()
        )
        return serie[:20]
    ...
    def _tfhka_get_affected_invoice_number(self):
        ...
        if not affected_move:
            return ""
        return re.sub(r"[^0-9]", "", affected_move.name or "")
