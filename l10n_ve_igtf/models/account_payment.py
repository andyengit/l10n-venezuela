...
class AccountPaymentIgtf(models.Model):
    ...
    @api.depends('company_id.igtf_percentage')
    def _compute_igtf_percentage(self):
        for payment in self:
            payment.igtf_percentage = payment.company_id.igtf_percentage
    ...
    def _compute_is_igtf(self):
        for payment in self:
            payment.is_igtf_on_foreign_exchange = bool(
                payment.journal_id and payment.journal_id.is_igtf
            )

    @api.depends("amount")
    def _compute_igtf_amount(self):
        """
        Compute the IGTF amount.

        This method calculates the IGTF (Impuesto a las Grandes Transacciones Financieras) amount based on the
        payment amount and the IGTF percentage set for the company, provided the journal applies IGTF.

        For each payment:
        - If the IGTF amount is not already set, it initializes to 0.0.
        - If the journal linked to the payment applies IGTF, the IGTF amount is calculated as:
          `payment.amount * (payment.igtf_percentage / 100)`

        This ensures that the IGTF amount is always assigned a valid float value.
        """
        for payment in self:
            currency = payment.currency_id or payment.company_id.currency_id

            if payment.journal_id.is_igtf and payment.igtf_percentage and payment.amount:
                base = payment.amount
                payment.igtf_amount = currency.round(
                    base * (payment.igtf_percentage / 100.0)
                )
            else:
